...
This can also be overriden by specifying the `EntityAttributes.LINK_TO_DETAIL_VIEW` attribute on the `EntityConfiguration`, or the current `EntityViewRequest`.
