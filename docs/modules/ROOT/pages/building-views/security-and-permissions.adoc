...
| xref:building-views/form-view.adoc#create-update-view[Create view]
| `AllowableAction.CREATE`
| global

| xref:building-views/form-view.adoc#readonly-view[Readonly view]
| `AllowableAction.READ`
| instance

| xref:building-views/form-view.adoc#create-update-view[Update view]
...
These links depend on the presence of the required action, except for readonly views:

- If access to the readonly view is allowed, but not to the update view, a link to the readonly view will be available.
