...
* Added support for xref:building-views/list-view.adoc#securing-list-view[filtering a list view based on the presence of an `AllowableAction`]. (For example only showing items that have the `AllowableAction.READ` for the current user.)
