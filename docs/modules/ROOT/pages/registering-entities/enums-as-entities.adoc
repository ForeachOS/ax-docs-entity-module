...
When creating an `EntityConfiguration` for an enum, a basic `EntityModel` will get built and all enum properties will be configurable.
...
== Labels
The labels for enum properties can be set using xref:services-and-components/message-codes.adoc[message codes].
