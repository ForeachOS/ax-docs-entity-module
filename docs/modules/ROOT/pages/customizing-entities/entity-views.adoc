...
More information can be found in the xref:customizing-entities/index.adoc#_configuring_labels[Customizing existing entities] section.

See also the section on xref:building-views/customizing-views/form-view.adoc#configuring-form-text[configuring form controls text] for common customizations.
...
=== Nested properties
Property names can also be a selection of nested properties.
To select a nested property you can use the `.` notation e.g. `author.name`.
When rendering a nested property, `EntityModule` renders it as an embedded object by rendering a fieldSet around your property.
